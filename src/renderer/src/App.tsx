...
function App() {
  ...
  // Coordinate window resize with content transition
  useEffect(() => {
    ...
    <AnimatePresence mode="wait">
      {isRunning ? (
        <motion.div 
          ...
          className="h-[40px] w-[100px] bg-zinc-950/80 backdrop-blur-md border border-white/10 rounded-full flex items-center justify-center relative overflow-hidden draggable group select-none shadow-[0_4px_24px_-8px_rgba(0,0,0,0.5),inset_0_1px_0_rgba(255,255,255,0.1)]"
        >
          {/* Progress Bar Background */}
          <motion.div 
            className="absolute bottom-0 left-0 h-full bg-white/10 pointer-events-none"
            style={{ width: `${progress * 100}%` }}
            layoutId="progress-bar-bg"
          />
          
          {/* Clickable Area */}
          <button 
            onClick={toggle}
            className="relative z-10 nodrag cursor-pointer flex items-center justify-center w-full h-full outline-none focus:outline-none group/btn"
          >
            <div className="pointer-events-none transition-transform group-hover/btn:scale-90 duration-300">
        ...
        <motion.div 
          ...
          className="bg-zinc-950 text-white min-h-[100dvh] transition-colors duration-700 overflow-x-hidden relative"
        >
          ...
          <main className="w-full px-[4vw] py-[8vw]">
            <div className="grid items-start grid-cols-1 min-[638px]:grid-cols-[1.5fr_1fr] gap-[8vw] min-[638px]:gap-[6vw] w-full">
              ...
              <section className="flex flex-col items-start w-full min-w-0">
                ...
                <div className="w-full relative overflow-hidden bg-zinc-900/40 border border-white/5 rounded-[clamp(1rem,4vw,3rem)] p-[max(2rem,6vw)] backdrop-blur-xl shadow-2xl shadow-black/40">
                  {/* Progress Bar */}
                  <motion.div 
              ...
              <aside className="flex flex-col gap-[3vw] mt-0 min-[638px]:mt-[max(40px,8vw)] w-full nodrag min-w-0">
                ...
                <div className="bg-zinc-900/40 backdrop-blur-xl border border-white/5 rounded-[clamp(1rem,3vw,2.5rem)] p-[3vw] shadow-lg">
